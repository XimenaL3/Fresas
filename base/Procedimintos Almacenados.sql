...
BEGIN
    ...
    -- Cursor para recorrer los detalles de la devolución
    ...
    -- Recorrer cada devolución para actualizar Finanzas e inventario
    OPEN cur;
    read_loop: LOOP
        FETCH cur INTO v_idVenta, v_idPlatillo, v_CantidadDevuelta, v_TotalDevuelto;
        IF done = 1 THEN
            LEAVE read_loop;
        END IF;

        -- Actualizar total de venta en Finanzas
        UPDATE Finanzas
        SET TotalVenta = TotalVenta - v_TotalDevuelto
        WHERE idVenta = v_idVenta;

        -- Regresar ingredientes al inventario
        UPDATE Ingrediente i
        ...
        -- Validación: no permitir ventas con total negativo
        IF EXISTS (SELECT 1 FROM Finanzas WHERE idVenta = v_idVenta AND TotalVenta < 0) THEN
            SIGNAL SQLSTATE '45000'
